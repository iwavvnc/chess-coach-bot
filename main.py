...
# --- БЫСТРЫЙ ДЕТЕКТОР КОНЦЕПЦИЙ ---
def analyze_board_concepts(board: chess.Board) -> list:
    detected = []

    # 1. Поиск незащищенных фигур
    undefended = 0
    ...
    if undefended >= 1:
        detected.append({
            "topic": "🎯 **Зависающие фигуры:** Оставление своих фигур без защиты.",
            "query": "незащищенные фигуры зависающие фигуры шахматы"
        })

    # 2. Слабость последней горизонтали
    for color, sqs in [(chess.WHITE, [chess.F1, chess.G1, chess.H1]), (chess.BLACK, [chess.F8, chess.G8, chess.H8])]:
        if board.king(color) in [chess.G1, chess.H1, chess.G8, chess.H8]:
            if sum(1 for sq in sqs if board.piece_at(sq) == chess.Piece(chess.PAWN, color)) == 3:
                detected.append({
                    "topic": "🎯 **Слабость последней горизонтали:** Отсутствие «форточки» для короля.",
                    "query": "мат по последней горизонтали форточка шахматы"
                })
                break

    # 3. Тактическая связка
    if board.pieces(chess.QUEEN, chess.WHITE) or board.pieces(chess.QUEEN, chess.BLACK):
        detected.append({
            "topic": "🎯 **Тактика Связка и Шампур:** Уязвимость фигур на одной линии.",
            "query": "тактический прием связка шампур шахматы"
        })

    return detected

# --- АСИНХРОННОЕ СКАЧИВАНИЕ ПАРТИЙ ---
...
async def analyze_player(message: types.Message):
    ...
    # 1. Быстрое асинхронное скачивание последних 10 партий
    ...
    # 2. Мгновенная экспресс-проверка позиций
    for game in games:
        ...
        if pgn_text:
            ...
            if pgn:
                ...
                if len(moves) > 10:
                    board = pgn.board()
                    # Смотрим позицию на середине партии
    ...
    # Ограничиваем список до 3 важнейших проблем
    ...
    if not detected_issues:
        ...
        yt_queries = ["расчет вариантов шахматы упражнения"]

    # 3. Подгрузка YouTube видео
    all_videos = []
    for q in yt_queries:
